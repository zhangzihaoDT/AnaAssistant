...
[
  ...
  },
  {
    ...
    "updated_at": "2026-04-29 00:00",
    "purpose": "按 series_group_logic 分组计算每日 locks 与 mid（first_assign_lock_time 中位数，天），并输出两张对比折线图（locks/mid）。上市前（按 time_periods.<model>.end）不绘制折线。",
    "inputs": [
      "schema/data_path.md（订单分析必需）",
      "schema/business_definition.json（series_group_logic/time_periods）"
    ],
    "outputs": [
      "scripts/reports/series_group_logic_trend_by_date.html（默认）",
      "out/*.csv（可选，--csv-out）"
    ]
  },
  {
...
- 车系趋势对比（locks/mid）：`python3 scripts/series_group_logic_trend_by_date.py --models CM2 LS8`
...
flowchart TB
  ...
  data_path --> series_trend[series_group_logic_trend_by_date.py]
  business_def --> series_trend
  series_trend --> series_trend_html[scripts/reports/series_group_logic_trend_by_date.html]
