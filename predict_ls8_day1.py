...
historical_data = {
    "CM0": {"presale_top3_retained": 3907, "presale_top7_retained": 5178, "total_retained": 24399, "total_locks": 11577, "listing_day1_locks": 2236},
    "DM0": {"presale_top3_retained": 3471, "presale_top7_retained": 4201, "total_retained": 14845, "total_locks": 2664, "listing_day1_locks": 761},
    "CM1": {"presale_top3_retained": 2761, "presale_top7_retained": 4099, "total_retained": 21113, "total_locks": 5961, "listing_day1_locks": 1712},
    "DM1": {"presale_top3_retained": 3083, "presale_top7_retained": 4260, "total_retained": 14991, "total_locks": 4487, "listing_day1_locks": 1322},
    "CM2": {"presale_top3_retained": 7297, "presale_top7_retained": 9856, "total_retained": 49993, "total_locks": 19660, "listing_day1_locks": 5977},
    "LS9": {"presale_top3_retained": 4376, "presale_top7_retained": 6389, "total_retained": 8849, "total_locks": 3319, "listing_day1_locks": 1782},
}

print(f"\n【已知事实】LS8 预售前3日留存小订数: {ls8_top3_orders} | 前7日留存小订数: {ls8_top7_orders}")
print("\n--- 预测 LS8 上市后【首日(Day1)】锁单数 (基于【全周期30日总锁单】倒推) ---")
for model_name, stats in historical_data.items():
    ...
    listing_day1_ratio = stats['listing_day1_locks'] / stats['total_locks']
    pred_listing_day1_locks = pred_locks_30d * listing_day1_ratio
    
    print(f"[对标 {model_name}]")
    print(f"  -> 预测 30日总锁单: {int(pred_locks_30d)}")
    print(f"  -> 假定 上市首日锁单 占 30日总锁单 的比例: {listing_day1_ratio*100:.1f}%")
    print(f"  => 预测 LS8 上市后首日(Day1)总锁单: {int(pred_listing_day1_locks)}")
...
print("\n--- 基于一元线性回归模型 (预测30日总锁单数 -> 上市首日锁单数) ---")
print(f"方程: 上市首日锁单 = {slope:.2f} * 30日总锁单数 + {intercept:.2f}")
...
print(f"-> 预测 LS8 上市后首日(Day1)锁单: {int(pred_reg)}")
