...
def _calc_assign_metrics(
    ...
    df = pd.read_csv(assign_path, encoding="utf-16", sep="\t")
    ...
    def _sum(col: str) -> int:
        ...
        if col2 is None:
            return 0
        return int(pd.to_numeric(day[col2], errors="coerce").fillna(0).sum())
...
def _calc_test_drive_metrics(test_drive_path: Path, target_date: pd.Timestamp) -> dict[str, int]:
    df = pd.read_csv(test_drive_path, encoding="utf-16", sep="\t")
    df["_date"] = _parse_cn_ymd_series(df["create_date 年/月/日"])
    day = df[df["_date"] == target_date.normalize()]
    effective = int(pd.to_numeric(day.get("有效试驾数"), errors="coerce").fillna(0).sum())
